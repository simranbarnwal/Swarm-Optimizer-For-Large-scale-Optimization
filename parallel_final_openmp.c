...
int main()
{
	...
	clock_t start, end; // to find time taken by serial code
    ...
    {       
        ...
        for(int i = nl; i>=3; i--)
        {
            #pragma omp parallel default( shared )private(j,l1,l2,temp,x1,x2, a1, a2, a3, xij)
            {	
            	#pragma omp for
				for(j = 1; j<=ls; j++)
		        {
		            ...
		            x1 = index[r1(l[l1-1]+1,l[l1])];
		            x2 = index[r1(l[l2-1]+1,l[l2])];
		            a1 = r2();
		            a2 = r2();
		            a3 = r2();
        ...
        #pragma omp parallel default( shared )private(j,temp,x1,x2, a1, a2, a3, x2j)
        {	
        	#pragma omp for
		    for(j = 1; j<=ls; j++)
		    {
		        ...
		        // randomly select a particle in each level
		        x1 = index[r1(l[0]+1,l[1])];
		        x2 = index[r1(l[0]+1,l[1])];
